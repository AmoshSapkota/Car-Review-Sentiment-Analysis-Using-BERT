...
def generate_adversarial_example(model, input_ids, attention_mask, labels, epsilon):
    model.eval()

    # Ensure input_ids is a long tensor
    ...
    output = model(inputs_embeds=embeddings, attention_mask=attention_mask, labels=labels)
    loss = output.loss
    ...
    with torch.no_grad():
        embeddings_adv = embeddings + epsilon * embeddings.grad.sign()

    return embeddings_adv
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}"):
            ...
            if epsilon:
                ...
                embeddings = generate_adversarial_example(model, input_ids, attention_mask, labels, epsilon)
...
train_encodings_bert = tokenizer_bert(list(train_df['Review']),
                                      truncation=True, padding=True, return_tensors='pt')
test_encodings_bert = tokenizer_bert(list(test_df['Review']),
                                     truncation=True, padding=True, return_tensors='pt')
...
train_encodings_wwm = tokenizer_wwm(list(train_df['Review']),
                                    truncation=True, padding=True, return_tensors='pt')
test_encodings_wwm = tokenizer_wwm(list(test_df['Review']),
                                   truncation=True, padding=True, return_tensors='pt')
